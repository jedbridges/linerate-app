...
**Perspective device frame** (`tilt-frame.tsx` + the `.lr-tilt` / `.lr-frame` utilities): a panel that holds scrollable content, tilted back toward the horizon and snapping upright as it scrolls into place via the `lr-untilt` keyframe on a `view()` timeline (flat and upright where scroll timelines or motion are unavailable). It is 16:9 from `lg` up (where `DashboardShell` shows its full desktop layout) and portrait (3:4) below, matching the dashboard's own single-column collapse, so the console reads well from phone to desktop and scrolls inside the frame. (`DashboardShell` and `AppTopNav` also carry a `compact` prop that collapses to a single-column portrait layout for narrow contexts, unused by the current landscape frame.)
